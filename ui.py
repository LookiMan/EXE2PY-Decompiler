...
class GifPlayer(QtCore.QObject):
    def __init__(self, widget):
        ...
        self.banner.setGeometry(0, 26, self.centralwidget.width(), self.centralwidget.height())
        ...
        self.movie_screen.setGeometry(QtCore.QRect(self.centralwidget.width()//2-62, self.centralwidget.height()//2-62, 124, 124))
...
class Thread(QtCore.QThread):
    ...
    def run(self):
        """ """
        # 
        status_code = self.function(*self.args)
        # 
        self.callback.emit(status_code)
...
class MainWindow(BaseForm):
    ...
    def __init__(self):
        ...
        self.form.setStyleSheet('#MainWindow{background-color: #F2F2F2; border: 1 solid #000;}')
        ...
        self.file_search_button.setStyleSheet("#file_search_button{"\
        ...
        self.stop_button.setStyleSheet("#stop_button{"\
    ...
    def stop_processing(self, status_code):
        ...
        if status_code in range(200, 300):
            ...
            show_error('Критическая ошибка', 'Непредвиденый статус код: %s' % str(status_code))
